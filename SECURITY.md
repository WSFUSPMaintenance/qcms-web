...
The replacement snapshot publisher is designed for GitHub Actions workload identity federation. It exchanges the runner's short-lived OIDC assertion for a Microsoft Graph token and requires an Entra application restricted to this repository environment and `Sites.Selected` permission for the QCMS site. Do not grant tenant-wide SharePoint read access when the site-specific grant is sufficient.
