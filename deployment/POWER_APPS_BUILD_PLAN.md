...
Current connected data sources are QCMS Departments, Form Catalog, Checklist Items, Inspection Records, Inspection Responses, Escalation Log, Signature Log, Inspection Schedule, Corrective Actions, Workflow History, Web Submission Bridge, and App Roles. Corrective Actions is used by the Department Manager workspace; Inspection Schedule, Workflow History, and Web Submission Bridge remain intentionally unused until their screens are implemented. App Roles now provides the governed Administrator capability source.
...
Six home KPI delegation warnings remain. They are not functional errors, but production scale requires maintained aggregate values or targeted flows rather than client-side `CountRows(Filter(...))` scans. `App.OnStart` now derives Department Manager and QA Reviewer capabilities from the active Department routing list, derives Administrator capability from active `QCMS App Roles` rows, and initializes draft version and Central-time metadata. Additional small-reference caching remains pending.
...
`QCMS App Roles` is the application-role registry. It contains optional `UserEmail` and `Role` text fields and an `Active` Yes/No field. Administrator authorization requires an active row whose normalized `UserEmail` matches the authenticated user and whose `Role` is exactly `Administrator`. Keep the registry empty until an authorized business owner explicitly assigns the initial administrator. App-role checks govern navigation only and never replace SharePoint list permissions.
...
3. Derive Inspector, Department Manager, and QA Reviewer capabilities from Department routing, and derive Administrator capability from active `QCMS App Roles` rows; UI role checks do not replace SharePoint permissions.
